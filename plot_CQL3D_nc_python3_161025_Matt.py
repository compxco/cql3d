...
ios='linux' #Will be changed to 'linux', if Python detects it, see below. 
...
file_cql3d='eoved_.16_multi-flux-surface.test1.0.nc' # Simple test case
...
DCmx= 0. #1.e16 # If DCmn=0. and DCmx=0., the limits will be set automatically
...
imx=1.0 #0.125 #0.5 #1/sqrt(10) #0.07 #1.0 #0.75    # limits for u_par/unorm and u_prr/unorm 
...
if data_src=='DC_text':
    ...
    print('========================================')
    print("The global attributes: ",list(s_file_cql3d.dimensions.keys()))        
    print("File contains variables: ",list(s_file_cql3d.variables.keys()))
    print('========================================')
...
#            plt.plot(rya,np.asarray(powrfl[Nt-1,i,:]),'k',linewidth=linw)
        ...
        powden_e_Nt=powden_e[Nt-1,:] # last time step, 1st species
        powden_i_Nt=powden_i[Nt-1,:] # last time step, 1st species
        ...
        if np.amax(powden_mx)>0.01:
            ...
            for it in range(0,Nt,it_stride):
                it_count=it+1
                powden_e_it=powden_e[it,:] #each t step, 1st species (k-1=0)
        ...
        if np.amax(powden_mx)>0.01:
            ...
            for it in range(0,Nt,it_stride):
                it_count=it+1
                powden_i_it=powden_i[it,:] #each t step, 1st species (k-1=0)
...
if iplot_fpar==1:
    ...
    gammal= sqrt(1+(xl*unorm)**2/clight**2)  #
    Vpar_c= (xl*unorm/clight)/gammal # == (u_par/c)/gamma = Vpar/c
    #print 'lrz=',lrz
    #print 'jfl=',jfl
    fl_mx=np.max(fl)
    ...
    savefig('Fpar_vs_upar'+'.png') # Save as png plot
    # Save as eps plot:
    if isave_eps==1: 
        savefig('Fpar_vs_upar'+'.eps')

    fig1= plt.figure() #------------ Fpar(Vpar/c) distr.func
    ax = plt.subplot(1, 1, 1)
    ###plt.hold(True)
    plt.grid(True)
    plt.xlim((-1.1,+1.1))   
    ...
    for lr in range(0,lrz,1):
        ...
        if fl_lr_mx>0.0:
            ...
            #print 'lr=',lr,'  ir=',ir            
            plt.yscale('log') # log10 scale !Comment if you want a linear scale
            plt.plot(Vpar_c[:],fl_lr[:],color=col,linewidth=linw)
    plt.minorticks_on() # To add minor ticks
    plt.tick_params(which='both',  width=1)
    plt.tick_params(which='major', length=7)
    plt.tick_params(which='minor', length=4, color='k') 
    plt.title("$F_{||}$  $(normalized$ $units)$",y=1.03)    
    xlabel(r"$V_{||}/c$",fontsize=fnt+6)        
    savefig('Fpar_vs_Vpar'+'.png') # Save as png plot
    # Save as eps plot:
    if isave_eps==1: 
        savefig('Fpar_vs_Vpar'+'.eps')
    #-------------------- FIGURE IS SAVED -------------------------
...
#For 3D plots, check http://matplotlib.sourceforge.net/mpl_toolkits/mplot3d/tutorial.html
    ...
    if plot_type=='c':  #-> CONTOUR plot 
        ...
        if DCmn==0. and DCmx==0.: 
            ...
            DCmax = DCmx
        if Coeff=='f' or Coeff=='f_local':
            DCmin=10 #6.9 #8 #6 #6 #4 #-4 #-50 #5  # log10(f) lowest level
            DCmax=18. #16.5 #18 #16 #15 #17 # log10(f) highest level
        if DCmin==DCmax:
            DCmin=0
            DCmax=1
...
#                                     '-',color=col,linewidth=linw2)
        ...
        #plt.xticks((ucmx/10*np.array([-10,-8,-6,-4,-2,0,2,4,6,8,10])).tolist()) # 1000keV for electrons: u/c=2.8
        #plt.yticks((ucmx/10*np.array([0,2,4,6,8,10]).tolist()))
        #xticksp = (1.1*ucmx/10*np.array([-10,-8,-6,-4,-2,0,2,4,6,8,10])).tolist()
        #yticksp = (1.1*ucmx/10*np.array([0,2,4,6,8,10])).tolist()
        #plt.xticks(xticksp) # 1000keV for electrons: u/c=2.8
        #plt.yticks(yticksp)
#=============================================================================
        if ucmx<0.015:
